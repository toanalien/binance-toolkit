...
iso_symbol_has_asset = list(
    filter(
        lambda x: x["baseAsset"]["totalAsset"] != "0" or x["quoteAsset"][
            "totalAsset"] != "0",
        margin_iso["assets"],
    ))
...
list(
    map(
        lambda x: (
            print("{:>10}{:>15}{:>15}{:>15}{:>15}{:>15}{:>15}{:>15}{:>15}".
                  format(
                      x["baseAsset"]["asset"],
...
intrade_symbol = (
    intrade_symbol + list(map(lambda x: x["symbol"], iso_symbol_has_asset)) +
    list(map(lambda x: x["symbol"], margin_openorders)) + list(
        map(
            lambda x: f"{x['asset']}USDT"
            if not x["asset"].replace("/", "") in ["USDT"] else "",
            cro_symbol_has_asset,
        )))
...
print("-" * 156)
print(
    "{:>10}{:>15}{:>10}{:>15}{:>10}{:>10} {:>6}{:>10}{:>10}{:>15}{:>15}{:>23}{:>23}"
    .format(
        "symbol",
        "orderId",
        "origQty",
...
list(
    map(
        lambda x: (
            x[1].update({
                "price":
                float(x[1]["cummulativeQuoteQty"]) / float(x[1]["executedQty"])
            }) if x[1]["type"] == "MARKET" else None,
            print(
                "{:>10}{:>15}{:>10}{:>15}{:>10}{:10.2f}%{:>6}{:>10}{:10.2f}{:>15}{:>15}{:>23}{:>23}"
                .format(
                    x[1]["symbol"],
                    x[1]["orderId"],
                    x[1]["origQty"],
                    x[1]["executedQty"],
                    x[1]["price"],
                    (float(dict_ticker_price[x[1]["symbol"]]["last"]) - float(
                        x[1]["price"])) / float(x[1]["price"]) *
                    (-1 if x[1]["side"] == "SELL" else 1) * 100,
                    x[1]["side"],
                    x[1]["status"],
                    float(x[1]["executedQty"]) * float(x[1]["price"])
                    if x[1]["side"] == "FILLED" else float(x[1]["origQty"]) *
                    float(dict_ticker_price[x[1]["symbol"]]["last"]),
                    x[1]["stopPrice"],
                    dict_ticker_price[x[1]["symbol"]]["last"],
                    datetime.fromtimestamp(x[1]["time"] / 1000.0).strftime(
                        "%d/%m/%Y, %H:%M:%S%z"),
                    datetime.fromtimestamp(x[1]["updateTime"] / 1000.0
                                           ).strftime("%d/%m/%Y, %H:%M:%S%z"),
                )),
            print("-" * 10)
            if x[0] != len(margin_all_closed_orders) - 1 and x[1]["symbol"] !=
            margin_all_closed_orders[x[0] + 1]["symbol"] else None,
        ),
